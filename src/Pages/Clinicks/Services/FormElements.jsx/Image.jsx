...
const Image = ({field,setFormFields,i,formFields}) => {
    ...
    const [selectedOption, setSelectedOption] = useState(field?.fileType || 'All');
    const [sizeIn, setSizeIn] = useState(field?.sizeIn || 'KB');
    const [ minSize, setMinSize ] = useState(field?.minSize || '');
    const [ maxSize, setMaxSize ] = useState(field?.maxSize || '');
    const [ minLimit, setMinLimit ] = useState(field?.minLimit || 1);
    const [ maxLimit, setMaxLimit ] = useState(field?.maxLimit || 1);
    ...
    const handleSubmit = (e)=>{
        ...
        setFormFields((prev) =>prev.map((field, index) =>
            index === i ? { ...field, label : title,minLimit,maxLimit,sizeIn,minSize,maxSize,fileType : selectedOption,hide: hide,mandatory : mandatory,instruction : instruction } : field
          ))
          closeModal();
  ...
  return (
    ...
    <div tabIndex={1} className=' my-1 w-full hover:border-2 cursor-pointer hover:m-3 focus:hover:m-0 hover:border-dashed hover:border-gray-200 focus:hover:border-solid focus:border-orange-100 focus:border-[0.02px] focus:bg-orange-600 focus:rounded-xl focus:border-l-8 focus:border-l-orange-600 focus:bg-opacity-5 focus:hover:border-orange-100 focus:hover:border-[0.02px] focus:hover:bg-orange-600 focus:hover:rounded-xl focus:hover:border-l-8 focus:hover:border-l-orange-600 focus:hover:bg-opacity-5 flex flex-col p-5 ' >
          ...
          <div className="border border-dashed border-gray-500 mt-1 relative">
    <input type="file" disabled className="cursor-pointer relative block opacity-0 w-full h-full p-10  z-50"/>
    <div className="text-center p-10 absolute top-0 right-0 left-0 m-auto">
        <div className=' justify-center flex w-full'>
        <svg className='p-2 rounded-full bg-white' width="29" height="29" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M10.3715 3.1753L7.34234 0.146165H7.34311C7.24954 0.054141 7.12503 0.00232861 6.99435 8.40955e-06C6.86289 -0.000764859 6.73683 0.0518209 6.64404 0.144619L3.61491 3.17375C3.42777 3.36785 3.43086 3.67641 3.62187 3.86741C3.81288 4.05765 4.1222 4.05997 4.31554 3.87205L6.4932 1.69439V12.2919C6.4932 12.5649 6.71515 12.7868 6.98813 12.7868C7.26188 12.7868 7.48305 12.5649 7.48305 12.2919V1.68885L9.67081 3.86651C9.86337 4.05829 10.175 4.05829 10.3676 3.86651C10.5555 3.67473 10.5571 3.3694 10.3715 3.1753Z" fill="black"/>
<path d="M13.4928 11.5531C13.2198 11.5531 12.9979 11.775 12.9979 12.048V13.0162H0.989847V12.0418C0.989847 11.7688 0.767904 11.5469 0.494924 11.5469C0.221172 11.5469 0 11.7688 0 12.0418V13.5049C0 13.6364 0.0518125 13.7624 0.144611 13.8552C0.237409 13.948 0.363466 13.9998 0.494918 13.9998H13.4928C13.6243 13.9998 13.7503 13.948 13.8431 13.8552C13.9359 13.7624 13.9877 13.6364 13.9877 13.5049V12.0418C13.9846 11.7711 13.7642 11.5531 13.4928 11.5531Z" fill="black"/>
</svg>
        ...
        {options && <div className="-mr-36 ml-3 my-auto ">
    ...
    {showModal && (
                <div  className="fixed top-0 left-0 w-full scroll-none overflow-y-auto overflow-x-auto h-full flex items-center justify-center bg-gray-700 bg-opacity-80">
                    <div className="relative bg-white rounded-2xl w-3/5 scroll-none overflow-y-auto top-36">
                        ...
                        <div className='px-8 '>
                            ...
                            <div className=''>
                                ...
                                <input value={title} onChange={(e)=> setTitle(e.target.value)} className='rounded-lg w-full my-3 outline-1 outline-gray-100 p-4 border border-gray-50 px-6' />
                            ...
                            <div className=''>
                                ...
                                <textarea value={instruction} onChange={(e)=>setInstruction(e.target.value)} className='rounded-lg w-full my-3 outline-1 outline-gray-100 p-4 border px-6  border-gray-50' ></textarea>
                                   
                            </div>
                            <div className='flex flex-col mb-4'>
                            <label htmlFor="dropdown" className='font-semibold'>Image Upload Limit</label>
                            <div className='flex w-2/3'>
                                <div className='flex'>
                                <input type="text" disabled placeholder='Min' className='bg-white rounded-lg w-full my-3 outline-1 outline-gray-100 p-2 border border-gray-50 px-6'/>
                            <select className='rounded-lg w-1/3 -ml-10 my-3 outline-1 outline-gray-200  border border-gray-100 px-2' id="dropdown" value={minLimit} onChange={(e)=>setMinLimit(e.target.value)}>
                            {Array.from({ length: 10 }, (_, index) => (
                                ...
                                <div className='flex'>
                                <input type="text" disabled placeholder='Max' className='ml-3 bg-white rounded-lg w-full my-3 outline-1 outline-gray-100 p-2 border border-gray-50 px-6'/>
                            <select className='rounded-lg w-1/3 -ml-10 my-3 outline-1 outline-gray-200  border border-gray-100 px-2' id="dropdown" value={maxLimit} onChange={(e)=>setMaxLimit(e.target.value)}>
                            {Array.from({ length: 10 }, (_, index) => (
                                <option key={index} value={index + 1}>
                                {index + 1}
                                </option>
                            ...
                            <div className='flex flex-col mb-4'>
                            <label htmlFor="dropdown" className='font-semibold'>File Size</label>
                            <div className='flex w-2/3'>
                                <div className='flex'>
                                <input type="text" placeholder='Min' onChange={(e)=>setMinSize(e.target.value)} value={minSize} inputMode='numeric' pattern="[0-9]*" className='rounded-lg w-full my-3 outline-1 outline-gray-100 p-2 border border-gray-50 px-6'/>
                            <select className='rounded-lg w-1/3 -ml-10 my-3 outline-1 outline-gray-200  border border-gray-100 px-2' id="dropdown" value={sizeIn} onChange={(e)=>setSizeIn(e.target.value)}>
                                <option value="KB">KB</option>
                                <option value="MB">MB</option>
                            </select>
                                </div>
                                <div className='flex'>
                                <input type="text" onChange={(e)=>setMaxSize(e.target.value)} value={maxSize} placeholder='Max' inputMode='numeric' pattern="[0-9]*" className='ml-3 rounded-lg w-full my-3 outline-1 outline-gray-100 p-2 border border-gray-50 px-6'/>
                            <select className='rounded-lg w-1/3 -ml-10 my-3 outline-1 outline-gray-200  border border-gray-100 px-2' id="dropdown" value={sizeIn} onChange={(e)=>setSizeIn(e.target.value)}>
                                <option value="KB">KB</option>
                                <option value="MB">MB</option>
                            </select>
                            ...
                            <div className='flex flex-col mb-4'>
                            <label htmlFor="dropdown" className='font-semibold'>Image File Type</label>
                            <select className='rounded-lg w-2/3 my-2 outline-1 outline-gray-100 p-3 border border-gray-50 px-6' id="dropdown" value={selectedOption} onChange={handleSelectChange}>
                            ...
                            <div className=' col-start-2 ml-4 mb-5 col-span-4'>
